						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Table') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Tables", "action" => "index", "_full" => true],
...
<?php if (!empty($table->competitors)) : ?>
												<li><?= $this->Html->link(__('Competitors') . '...', ['controller' => 'Competitors', 'action' => 'index', 'parent', 'table', $table->id], ['class' => 'dropdown-item']) ?></li>
<?php endif ?>
											</ul>
										</li>
...
<?php } ?>
											<div class="row"><!-- 1. -->
												...
												<div class="col-sm-10 p-1 link">
													<?= $table->hasValue('competition') ? $this->Html->link($table->competition->name, ['controller' => 'Competitions', 'action' => 'view', $table->competition->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 2. -->
											...
											<div class="row"><!-- 3. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Tablejudge Id') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($table->tablejudge_id) ?><!-- 3.b -->
												</div>
											</div>
											<div class="row"><!-- 3. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Chairs') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($table->chairs) ?><!-- 3.b -->
												</div>
											</div>
<?php /*
											<div class="row"><!-- 6. -->
												...
												<div class="col-sm-10 p-1">
													<?= $this->Text->autoParagraph(h($table->description)) ?>
...
<?php if($config['show_counters']){ ?>
											<div class="row"><!-- counter helper -->
												...
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($table->competitor_count) ?><!-- 3.b -->
												</div>
											</div>
<?php } ?>
												...
												<div class="col-sm-12">
													<div class="row">
														<div id="readMoreDescription" class="col-sm-12 p-2 text read-more">
															<?= $this->Text->autoParagraph($table->description) ?>
...
<?php } ?>
											...
											<tbody>
												<?php foreach ($table->competitors as $competitors) : ?>
