...
<?php } ?>
											...
											<th class="integer chairman-id"><?= $this->Paginator->sort('chairman_id') ?></th><!-- H.3. -->
...
<?php if($config['show_counters']){ ?>
											<th class="number counter user_count"><?= $this->Paginator->sort('user_count') ?></th>											<th class="number counter competingclub_count"><?= $this->Paginator->sort('competingclub_count') ?></th><?php } ?>
...
<?php $name = $club->chairman->last_name . ' ' . $club->chairman->first_name; ?>
											<td class="string link chairman-id" value="<?= $club->chairman_id ?>"><?= $club->hasValue('country') ? $this->Html->link($name, ['controller' => 'Countries', 'action' => 'view', $club->chairman->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span></td>
											<td class="string email" value="<?= $club->email ?>"><?= h($club->email) ?></td>
											<td class="string web" value="<?= $club->web ?>"><?= h($club->web) ?></td>
<?php if($config['show_pos']){ ?>
...
<?php } ?>
<?php if($config['show_counters']){ ?>
											<td class="number counter user-count" value="<?= $club->user_count ?>"><?= h($club->user_count) ?></td>											<td class="number counter competingclub-count" value="<?= $club->competingclub_count ?>"><?= h($club->competingclub_count) ?></td><?php } ?>
<?php if($config['show_created'] || $config['show_modified']){ ?>
											<td class="datetime">
