 ...
 * @var iterable<\App\Model\Entity\Competingclub> $competingclubs
 */
use Cake\Core\Configure;

$layoutCompetingclubsLastId = -1;
if($session->check('Layout.Competingclubs.LastId')){
	$layoutCompetingclubsLastId = $session->read('Layout.Competingclubs.LastId');
}
...
?>
				<div class="competingclubs index row">
						...
						<div class="card">
							<div class="card-header">
								...
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-table fa-spin"></i> <?= __('Table') ?>: <?= __('Competingclubs') ?></h3>
									<div><?php
...
<?php if($config['show_counters']){ ?>
											<th class="number counter competitor_count"><?= $this->Paginator->sort('competitor_count') ?></th><?php } ?>
...
<?php } ?>
									...
									<tbody>
										<?php foreach ($competingclubs as $competingclub): ?>
<?php
	//$classLastVisited = ' class="last-visited"';	// later...
	//$classLastVisited = '';
?>

										<tr row-id="<?= $competingclub->id ?>"<?php if($competingclub->id == $layoutCompetingclubsLastId){ echo 'class="table-tr-last-id"'; } ?> prefix="<?= $prefix ?>" controller="<?= $controller ?>" action="<?= $action ?>" aria-expanded="true">
											<td class="row-id-anchor" value="<?= $competingclub->id ?>"><a name="<?= $competingclub->id ?>" class="anchor"></a></td>
<?php if($config['show_id']){ ?>
...
<?php } ?>
<?php if($config['show_visible']){ ?>
											<td class="boolean visible" value="<?= $competingclub->visible ?>"><?= h($competingclub->visible) ?></td>
<?php } ?>
<?php if($config['show_counters']){ ?>
											<td class="number counter competitor-count" value="<?= $competingclub->competitor_count ?>"><?= h($competingclub->competitor_count) ?></td><?php } ?>
<?php if($config['show_created'] || $config['show_modified']){ ?>
											<td class="datetime">
<?php if($config['show_created']){ ?>
												<span class="fw-bold"><?= h($competingclub->created) ?></span>
<?php } ?>
<?php if($config['show_created'] && $config['show_modified']){ ?>
												<br>
<?php } ?>
<?php if($config['show_modified']){ ?>
												<span class="fw-normal"><?= h($competingclub->modified) ?></span>
<?php } ?>
											</td>
<?php } ?>
<?php if($config['show_button_view'] || $config['show_button_edit'] || $config['show_button_delete'] ){ ?>
...
<?php if($config['show_button_view']){ ?>
												<?= $this->Html->link('<i class="fa fa-eye"></i>', ['action' => 'view', $competingclub->id], ['escape' => false, 'role' => 'button', 'class' => 'btn btn-warning btn-sm', 'data-toggle' => 'tooltip', 'data-placement' => 'top', 'title' => __('View this item'), 'data-original-title' => __('View this item')]) ?>
<?php } ?>

<?php if($config['show_button_edit']){ ?>
												<?= $this->Html->link('<i class="fa fa-edit"></i>', ['action' => 'edit', $competingclub->id], ['escape' => false, 'role' => 'button', 'class' => 'btn btn-primary btn-sm', 'data-toggle' => 'tooltip', 'data-placement' => 'top', 'title' => __('Edit this item'), 'data-original-title' => __('Edit this item')]) ?>
<?php } ?>

<?php if($config['show_button_delete']){ ?>
												<?= $this->Form->postLink('', ['action' => 'delete', $competingclub->id], ['class'=>'hide-postlink index-delete-button-class']) ?>
												<a href="javascript:;" class="btn btn-sm btn-danger postlink-delete" data-bs-tooltip="tooltip" data-bs-placement="top" title="<?= __("Delete this record!") ?>" text="<?= h($competingclub->name) ?>" subText="<?= __("You will not be able to revert this!") ?>" confirmButtonText="<?= __("Yes, delete it!") ?>" cancelButtonText="<?= __("Cancel") ?>"><i class="fa fa-minus"></i></a>
