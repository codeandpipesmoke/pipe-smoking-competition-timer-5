...
class CompetitorsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Competitors'));
		...
		$this->paginate['Competitors']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Competitors']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Competitors']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Competitors->find()
				->contain(['Competitions', 'Users', 'Competingclubs', 'Tables'])
				->where([
					//$conditions,
					'OR' => [
						'name LIKE' => '%' . $search .  '%',
			...
			$query = $this->Competitors->find()->contain(['Competitions', 'Users', 'Competingclubs', 'Tables'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Competitors']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Competitors']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$competitor = $this->Competitors->get((int) $id, contain: ['Competitions', 'Users', 'Competingclubs', 'Tables']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('competitor') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $competitor->name;
		$this->set(compact('competitor', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('competitor') . ' ' . __('record'));
        $competitor = $this->Competitors->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $competitor = $this->Competitors->patchEntity($competitor, $data);
			//dd($competitor);
			/*
				if(...){
					$competitor->setErrors('field', __('Message'));
				}
			*/
			//dd($competitor->getErrors());
            if (!$competitor->hasErrors() && $this->Competitors->save($competitor)) {
                //$this->Flash->success(__('The competitor has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $competitor->id);
            ...
            //$this->Flash->error(__('The competitor could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $competitions = $this->Competitors->Competitions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $users = $this->Competitors->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $competingclubs = $this->Competitors->Competingclubs->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $tables = $this->Competitors->Tables->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('competitor', 'competitions', 'users', 'competingclubs', 'tables'));
    }
    ...
    public function edit($id = null)
    {
		...
		try {
			$competitor = $this->Competitors->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('competitor') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$competitor->setError('field', __('Message'));
				}
			*/
			//dd($competitor->getErrors());
			if (!$competitor->hasErrors() && $this->Competitors->save($competitor)) {
        ...
        $tables = $this->Competitors->Tables->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $competitor->name;
        $this->set(compact('competitor', 'competitions', 'users', 'competingclubs', 'tables', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
		...
		try {
			$competitor = $this->Competitors->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->Competitors->delete($competitor)) {
			...
			//$this->Flash->success(__('The competitor has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The competitor could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
