...
class TemplatesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Templates'));
		...
		$this->paginate['Templates']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Templates']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Templates']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Templates->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Templates']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Templates']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$template = $this->Templates->get((int) $id, contain: ['Diplomas']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('template') . ' ' . __('record'));
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('template') . ' ' . __('record'));
        $template = $this->Templates->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $template = $this->Templates->patchEntity($template, $data);
			//dd($template);
			/*
				if(...){
					$template->setErrors('field', __('Message'));
				}
			*/
			//dd($template->getErrors());
            if (!$template->hasErrors() && $this->Templates->save($template)) {
                //$this->Flash->success(__('The template has been saved.'), ['plugin' => 'JeffAdmin5']);
            ...
            //$this->Flash->error(__('The template could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$template = $this->Templates->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('template') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$template->setError('field', __('Message'));
				}
			*/
			//dd($template->getErrors());
			if (!$template->hasErrors() && $this->Templates->save($template)) {
    ...
    public function delete($id = null)
    {
		...
		if ($this->Templates->delete($template)) {
			...
			//$this->Flash->success(__('The template has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The template could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
