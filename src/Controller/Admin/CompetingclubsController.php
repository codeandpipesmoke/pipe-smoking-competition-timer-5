...
/**
 * Competingclubs Controller
 *
 * @property \App\Model\Table\CompetingclubsTable $Competingclubs
 */
class CompetingclubsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Competingclubs'));
		...
		$this->paginate['Competingclubs']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Competingclubs']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Competingclubs']['order'] 	= $this->defaultOrder;
		}
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Competingclubs']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Competingclubs']['maxLimit'] = $this->config['paginate_limit'];
			$competingclubs = $this->paginate($query);
		...
		if(empty($competingclubs->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('competingclubs'));
    }

    /**
     * View method
     *
     * @param string|null $id Competingclub id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('competingclub') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $competingclub->name;
		$this->set(compact('competingclub', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('competingclub') . ' ' . __('record'));
        $competingclub = $this->Competingclubs->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $competingclub = $this->Competingclubs->patchEntity($competingclub, $data);
			//dd($competingclub);
			/*
				if(...){
					$competingclub->setErrors('field', __('Message'));
				}
			*/
			//dd($competingclub->getErrors());
            if (!$competingclub->hasErrors() && $this->Competingclubs->save($competingclub)) {
                //$this->Flash->success(__('The competingclub has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $competingclub->id);
    ...
    public function edit($id = null)
    {
		...
		try {
			$competingclub = $this->Competingclubs->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('competingclub') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$competingclub->setError('field', __('Message'));
				}
			*/
			//dd($competingclub->getErrors());
			if (!$competingclub->hasErrors() && $this->Competingclubs->save($competingclub)) {
    ...
    public function delete($id = null)
    {
		...
		try {
			$competingclub = $this->Competingclubs->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		if ($this->Competingclubs->delete($competingclub)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
			//$this->Flash->success(__('The competingclub has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The competingclub could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
