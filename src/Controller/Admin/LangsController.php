...
class LangsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Langs'));
		...
		$this->paginate['Langs']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Langs']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Langs']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Langs->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Langs']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Langs']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$lang = $this->Langs->get((int) $id, contain: ['Users' => ['Countries', 'Clubs']]);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('lang') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $lang->name;
		$this->set(compact('lang', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('lang') . ' ' . __('record'));
        $lang = $this->Langs->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $lang = $this->Langs->patchEntity($lang, $data);
			//dd($lang);
			/*
				if(...){
					$lang->setErrors('field', __('Message'));
				}
			*/
			//dd($lang->getErrors());
            if (!$lang->hasErrors() && $this->Langs->save($lang)) {
                //$this->Flash->success(__('The lang has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $lang->id);
            ...
            //$this->Flash->error(__('The lang could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$lang = $this->Langs->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('lang') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$lang->setError('field', __('Message'));
				}
			*/
			//dd($lang->getErrors());
			if (!$lang->hasErrors() && $this->Langs->save($lang)) {
    ...
    public function delete($id = null)
    {
		...
		if ($this->Langs->delete($lang)) {
			...
			//$this->Flash->success(__('The lang has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The lang could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
