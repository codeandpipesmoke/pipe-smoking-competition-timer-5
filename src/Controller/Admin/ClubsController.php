...
class ClubsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Clubs'));
		...
		$this->paginate['Clubs']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Clubs']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Clubs']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Clubs->find()->contain(['Countries', 'Chairmans'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Clubs']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Clubs']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('club') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $club->name;
		$this->set(compact('club', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('club') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$club->setErrors('field', __('Message'));
				}
			*/
			//dd($club->getErrors());
            if (!$club->hasErrors() && $this->Clubs->save($club)) {
                //$this->Flash->success(__('The club has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $club->id);
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('club') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$club->setError('field', __('Message'));
				}
			*/
			//dd($club->getErrors());
			if (!$club->hasErrors() && $this->Clubs->save($club)) {
    ...
    public function delete($id = null)
    {
		...
		if ($this->Clubs->delete($club)) {
			...
			//$this->Flash->success(__('The club has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The club could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
