...
class SetupsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		if($sort !== null && $direction !== null){
			...
			$this->paginate['Setups']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Setups->find()->contain(['Users'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		try {
			...
			$this->paginate['Setups']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$setup = $this->Setups->get((int) $id, contain: ['Users']);
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$setup->setErrors('field', __('Message'));
				}
			*/
			//dd($setup->getErrors());
            if (!$setup->hasErrors() && $this->Setups->save($setup)) {
        ...
        $users = $this->Setups->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
    ...
    public function edit($id = null)
    {
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$setup->setError('field', __('Message'));
				}
			*/
			//dd($setup->getErrors());
			if (!$setup->hasErrors() && $this->Setups->save($setup)) {
        ...
        $users = $this->Setups->Users->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $setup->name;
        $this->set(compact('setup', 'users', 'id', 'name'));
    }
