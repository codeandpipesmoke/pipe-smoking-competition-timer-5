...
/**
 * Chairmans Controller
 *
 * @property \App\Model\Table\ChairmansTable $Chairmans
 */
class ChairmansController extends AppController
{
	//public $defaultOrder 	 = ['Pagecategories.name' => 'asc', 'Pages.pos' => 'asc'];	// For example
	public $defaultOrder 	 = ['pos' => 'asc', 'name' => 'asc'];
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Chairmans'));
		...
		$this->paginate['Chairmans']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Chairmans']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Chairmans']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Chairmans->find()
				...
				->contain(['Clubs', 'Langs', 'Countries']);
		}else{
			$query = $this->Chairmans->find()->where($conditions)->contain(['Clubs', 'Langs', 'Countries']);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Chairmans']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Chairmans']['maxLimit'] = $this->config['paginate_limit'];
			$chairmans = $this->paginate($query);
		...
		if(empty($chairmans->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('chairmans'));
    }

    /**
     * View method
     *
     * @param string|null $id Chairman id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		Configure::write('Theme.admin.config.header_buttons_in_action.view', array_merge(Configure::read('Theme.admin.config.header_buttons_in_action.view'), 
			['back' => true, 'add' => false, 'edit' => false, 'save' => false, 'view' => false, 'delete' => false]
		));

		//dd($id);

		try {
			$chairman = $this->Chairmans->get((string) $id, contain: ['SocialAccounts', 'Clubs', 'Countries']);
			//dd($chairman);
		...
		$this->set('title', __('View the') . ': ' . __('chairman') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $chairman->name;
		$this->set(compact('chairman', 'id', 'name'));
    }
...
//		//));
//		
//		$this->set('title', __('Add new') . ': ' . __('chairman') . ' ' . __('record'));
//        $chairman = $this->Chairmans->newEmptyEntity();
//        if ($this->request->is('post')) {
//			$data = $this->request->getData();
//			//debug($data);
//            $chairman = $this->Chairmans->patchEntity($chairman, $data);
//			//dd($chairman);
//			/*
//				if(...){
//					$chairman->setErrors('field', __('Message'));
//				}
//			*/
//			//dd($chairman->getErrors());
//            if (!$chairman->hasErrors() && $this->Chairmans->save($chairman)) {
//                //$this->Flash->success(__('The chairman has been saved.'), ['plugin' => 'JeffAdmin5']);
//                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
//				$this->session->write('Layout.' . $this->controller . '.LastId', $chairman->id);
//
//                //return $this->redirect(['action' => 'add']);
//                return $this->redirect([
//					'controller' => $this->controller,
...
//
//            }
//            //$this->Flash->error(__('The chairman could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
//            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
//        }
//        $this->set(compact('chairman'));
//    }
//
//    /**
//     * Edit method
...
//
//		try {
//			$chairman = $this->Chairmans->get((int) $id, contain: []);
//		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
//			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
//			return $this->redirect(['action' => 'index']);
//		}
//
//		$this->set('title', __('Edit the') . ': ' . __('chairman') . ' ' . __('record'));
//		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
//			
...
//			/*
//				if(...){
//					$chairman->setError('field', __('Message'));
//				}
//			*/
//			//dd($chairman->getErrors());
//			if (!$chairman->hasErrors() && $this->Chairmans->save($chairman)) {
...
//
//		try {
//			$chairman = $this->Chairmans->get((int) $id);
//		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
//			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
//			return $this->redirect(['action' => 'index']);
//		}
//
//		if ($this->Chairmans->delete($chairman)) {
//			$this->session->delete('Layout.' . $this->controller . '.LastId');
//			//$this->Flash->success(__('The chairman has been deleted.'), ['plugin' => 'JeffAdmin5']);
//			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
//		} else {
//			//$this->Flash->error(__('The chairman could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
