...
class TablesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Tables'));
		...
		$this->paginate['Tables']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Tables']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Tables']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Tables->find()->contain(['Competitions'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Tables']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Tables']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$table = $this->Tables->get((int) $id, contain: ['Competitions', 'Competitors']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('table') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $table->name;
		$this->set(compact('table', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('table') . ' ' . __('record'));
        $table = $this->Tables->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $table = $this->Tables->patchEntity($table, $data);
			//dd($table);
			/*
				if(...){
					$table->setErrors('field', __('Message'));
				}
			*/
			//dd($table->getErrors());
            if (!$table->hasErrors() && $this->Tables->save($table)) {
                //$this->Flash->success(__('The table has been saved.'), ['plugin' => 'JeffAdmin5']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'JeffAdmin5']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $table->id);
            ...
            //$this->Flash->error(__('The table could not be saved. Please, try again.'), ['plugin' => 'JeffAdmin5']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'JeffAdmin5']);
        }
        $competitions = $this->Tables->Competitions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('table', 'competitions'));
    }
    ...
    public function edit($id = null)
    {
		...
		try {
			$table = $this->Tables->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'JeffAdmin5']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('table') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$table->setError('field', __('Message'));
				}
			*/
			//dd($table->getErrors());
			if (!$table->hasErrors() && $this->Tables->save($table)) {
        ...
        $competitions = $this->Tables->Competitions->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $table->name;
        $this->set(compact('table', 'competitions', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
		...
		if ($this->Tables->delete($table)) {
			...
			//$this->Flash->success(__('The table has been deleted.'), ['plugin' => 'JeffAdmin5']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'JeffAdmin5']);
		} else {
			//$this->Flash->error(__('The table could not be deleted. Please, try again.'), ['plugin' => 'JeffAdmin5']);
